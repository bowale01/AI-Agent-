...
def fetch_api_football_h2h(team1_id, team2_id):
...
def analyze_h2h_results(h2h_data, team1_name, team2_name):
    ...
    for match in h2h_data.get("response", []):
        score = match["teams"]
        if score["home"]["winner"]:
            winner = score["home"]["name"]
        elif score["away"]["winner"]:
            winner = score["away"]["name"]
        else:
            winner = None
    ...
    if team2_wins == 0:
        result["never_lost"] = f"{team2_name} has never beaten {team1_name}!"
    elif team1_wins == 0:
        result["never_lost"] = f"{team1_name} has never beaten {team2_name}!"
    return result
...
def automate_daily_h2h_analysis():
    ...
    for fixture in fixtures:
        ...
        h2h_data = fetch_api_football_h2h(team1_id, team2_id)
        if h2h_data:
            analysis = analyze_h2h_results(h2h_data, team1_name, team2_name)
            if analysis["never_lost"]:
                print(f"{team1_name} vs {team2_name}: {analysis['never_lost']}")
...
def save_results(results, filename="h2h_results.txt"):
    with open(filename, "w", encoding="utf-8") as f:
        for r in results:
            f.write(f"{r['team1']} vs {r['team2']}: {r['never_lost']}\n")
